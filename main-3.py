...
Concise Hindi mein sirf 3 plain lines do.
Koi star, asterisk, ya bold mat lagao.
Seedha likho:
1. Market mood
2. Best opportunity
3. Kya avoid karo
...
def main():
    ...
    # AI analysis alag message mein bhejo
    ai_text = get_ai_analysis(macro, global_mkts, news, signals_text, len(buy_signals), len(sell_signals), options)
    ai_report = "<b>AI ANALYSIS</b>\n" + ai_text + "\n\n<i>DYOR. Not financial advice.</i>"

    if len(report) > 3800:
        cut = report[:3700]
        last_newline = cut.rfind('\n')
        report = report[:last_newline] if last_newline > 3000 else cut

    send_telegram(report)

    # AI analysis alag bhejo — poora aayega!
