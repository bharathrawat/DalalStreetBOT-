...
# Sensex weekly expiry Friday hoti hai
...
# ✅ NEW: Sensex Options (BSE - Friday expiry)
def get_sensex_options():
    try:
        ...
        if data.get("status") and data.get("data"):
            return data["data"]
        print(f"Sensex options empty: {data}")
        return None
    except Exception as e:
        print(f"Sensex options error: {e}")
        return None

# ✅ NEW: Stock specific options data
...
# ✅ NEW: Chart generator for Top 3 Picks
    ...
    try:
        ...
        if signal_data.get('s1') and signal_data['s1'] != 'N/A':
            ax1.axhline(y=signal_data['s1'], color='#00ff88', linestyle='--',
                        linewidth=0.8, alpha=0.7, label=f"Support {signal_data['s1']}")
        if signal_data.get('r1') and signal_data['r1'] != 'N/A':
            ax1.axhline(y=signal_data['r1'], color='#ff4444', linestyle='--',
                        linewidth=0.8, alpha=0.7, label=f"Resist {signal_data['r1']}")
...
# ✅ NEW: Send chart image to Telegram
def send_telegram_photo(photo_buf, caption):
    try:
        ...
        requests.post(url, data={"chat_id": CHAT_ID, "caption": caption, "parse_mode": "HTML"},
                      files={"photo": ("chart.png", photo_buf, "image/png")}, timeout=15)
    except Exception as e:
...
def main():
    ...
    sensex_chain = get_sensex_options()
    sensex_options = analyze_options_chain(sensex_chain, sensex_spot)
    ...
    if top_picks:
        ...
        for i, s in enumerate(top_picks):
            ...
            if stock_opts:
                picks_report += f"📊 PCR: {stock_opts['pcr']} ({stock_opts['pcr_signal']})\n"
                picks_report += f"Max Pain: {stock_opts['max_pain']} | IV: {stock_opts['iv']}% {stock_opts['iv_signal']}\n"
                picks_report += f"ATM CE: {stock_opts['atm_ce']} | ATM PE: {stock_opts['atm_pe']}\n"
            picks_report += "\n"
        send_telegram(picks_report)

        # ✅ Send chart for each Top 3 pick
        for s in top_picks:
            ...
            if token:
                ...
                if not df_chart.empty:
                    ...
                    if chart_buf:
                        caption = f"📈 <b>{s['name']}</b> | {s['signal']} | RSI: {s['rsi']} | Conf: {s['confidence']}/10\nT1: {s['t1']} | SL: {s['sl']}"
